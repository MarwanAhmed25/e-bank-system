...
async function create(req: Request, res: Response) {
  ...
  try {
    ...
    if (!(u.name as unknown as string)) u.name = u.slug;
...
async function update(req: Request, res: Response) {
  ...
  try {
    ...
    if (perrmission && user.slug === slug) {
      const result = await user_obj.update(
        u.email,
        u.name,
        u.slug as unknown as string,
        u.phone,
        exist_user?.getDataValue('slug')
      );
      const token = jwt.sign({ user: result }, secret);
...
async function login(req: Request, res: Response) {
  ...
  try {
    ...
    if (status === 'suspended') return res.status(400).json('User suspended.');
