    ...
    if (permisson && user.role === 'admin') {
      ...
      if (account_number) {
        result = result.filter(
          (a) =>
            a.getDataValue('sender') === account_number ||
...
async function create(req: Request, res: Response) {
  ...
  try {
    ...
    if (!sender) return res.status(400).json('login please .');
    if (!reciver)
    ...
    if (permisson) {
      ...
      await account_obj.update(
        reciver_balance + l.amount,
        reciver?.getDataValue('userSlug')
      );
