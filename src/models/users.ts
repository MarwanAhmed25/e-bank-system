...
export class User {
  ...
  async create(u: user) {
    try {
      ...
      const hash = bcrypt.hashSync(
        u.password + config.extra_password,
        parseInt(config.password_round as string)
      );
      u.password = hash;
  ...
  async update(
    ...
    try {
      const result = await user_model.update(
        ...
        { where: { slug: old_slug }, returning: true }
      );
      console.log(result);
  ...
  async login(email: string, password: string) {
    try {
      ...
      const isTrue = await bcrypt.compare(
        password + config.extra_password,
        exist_password
      );
  ...
  async update_from_admin(accepted: boolean, status: string, slug: string) {
    try {
      const result = await user_model.update(
        { accepted, status },
        { where: { slug: slug }, returning: true }
      );
  ...
  async reset_password(password: string, slug: string) {
    try {
      ...
      password = hash;
      return await user_model.update(
        { password },
        { where: { slug: slug }, returning: true }
      );
    } catch (e) {
